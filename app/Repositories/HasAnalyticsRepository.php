...
class HasAnalyticsRepository extends Controller
{
     ...
     * @return int
     */
    public function handle($storeName)
    {
        $analytics = $this->initializeAnalytics();

        $property = $this->createProperty($analytics, $storeName);
        ...
        $data = [
            'view_id' => $property->id,
            'view_link' => $property->selfLink,
            'ga_store_id' => $property->accountId,
            'ga_tracking_id' => $property->webPropertyId,
     ...
     */
    public function createProperty($analytics, $storeName)
    {
        try {
            $property = new Google_Service_Analytics_Webproperty();
            // Get the storename the user has used as the name and use here
            $property->setName($storeName);
            $slugStoreName = Str::slug($storeName);

            $property->setWebsiteUrl("https://$slugStoreName.myduxstore.com");
            ...
            // dd($analytics, $property, $webProperty, $propertyID);
            return $this->createView($analytics, $propertyID, $storeName);
     ...
     */
    public function createView($analytics, $propertyTrackingID, $storeName)
    {
        // Construct the body of the request and set its properties.
        $profile = new Google_Service_Analytics_Profile();
        $viewName = "$storeName View";
        $profile->setName($viewName);

        $slugStoreName = Str::slug($storeName);

        $profile->setECommerceTracking(true);
        $profile->setWebsiteUrl("https://$slugStoreName.myduxstore.com");
        $profile->setTimezone('Africa/Lagos');
        $profile->setType('WEB');

        try {
            $view = $analytics->management_profiles->insert(self::$ACCOUNT_ID, $propertyTrackingID, $profile);
            // dd($view);
            return $view;
        } catch (apiServiceException $e) {
